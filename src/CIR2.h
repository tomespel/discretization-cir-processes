...
double psi(double k, double t) {                   //As defined in pag 10
	if (k != 0) { return (1 - exp(-k*t)) / k; }
	else return t;
}
...
vector<double> CIR2(int n, double x, double T, double k, double a, double sigma) { //Returns a vector with a sample from the second order scheme
	vector<double> v;
	...
	else {                 //Here we need to sample from Y and form a uniform(0,1), the scheme also depends on the threshold to be defined as in pag 11
		return v;
	}
}